...
public class DonorForgotPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            rs = st.executeQuery("select password from donor where email='" + to + "'");
            boolean a = rs.next();
            if(a) {
                String message=rs.getString("password");
                SendDonorPassword.send(to,message);
            ...
            else
            {
                out.println("<script>alert(\"Unable To Send Password,Try Again Later\");\n" +
                        " window.location=\"donorforgotpassword.html\";\n" +
                        "</script>");
            }
        }
        catch (Exception e)
